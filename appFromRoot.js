      ...
      if (foundItems.length === 0){
        Item.insertMany(defaultItems, function(err){
          if (err) {
            ...
            res.redirect("/"); // so it loops to else if items array is not empty, render the page
          }
        });
      } else {
        ...
        res.render("list", {listTitle: thisDayDate, newListItems: foundItems});
  ...
  DoList.findOne({doListName: customListName}, function(err, theDoList){
    if (err){
      ...
      if (!theDoList) {
        ...
        res.render("list", {listTitle: theDoList.doListName, newListItems: theDoList.doList });
